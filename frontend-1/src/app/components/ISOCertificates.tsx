...
export const ISOCertificates: React.FC<ISOCertificatesProps> = ({ variant = 'full' }) => {
  ...
  useEscapeKey(closeModal, !!selectedCertificate);

  const activateCert = (cert: ISOCertificate) => setSelectedCertificate(cert);
  const onCertKeyDown = (e: React.KeyboardEvent, cert: ISOCertificate) => {
      ...
      <div className="flex flex-wrap gap-3 items-center">
        {certificates.map((cert) => (
          <div
            ...
            role="button"
            tabIndex={0}
            aria-label={`View ${cert.number} certificate`}
            ...
            onClick={() => activateCert(cert)}
            onKeyDown={(e) => onCertKeyDown(e, cert)}
          >
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {certificates.map((cert) => (
          <div
            ...
            role="button"
            tabIndex={0}
            aria-label={`View ${cert.number} — ${cert.name}`}
      ...
      {/* Certificate Modal */}
      {selectedCertificate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={closeModal} role="presentation">
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="iso-cert-title"
            className="bg-card rounded-lg max-w-2xl w-full p-6 relative"
          ...
          >
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 id="iso-cert-title" className="text-2xl font-bold text-primary">{selectedCertificate.number}</h2>
