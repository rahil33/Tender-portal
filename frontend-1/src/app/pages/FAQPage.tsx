    ...
    questions: [
      {
        ...
        answer: 'We offer comprehensive services including GeM registration, tender bidding support, OEM panel setup, certificate assistance (MSME, ISO, etc.), catalogue management, and ongoing consultation.'
      },
      {
      ...
      },
      {
        ...
        answer: 'Yes, we provide comprehensive post-registration support including bid management, catalogue updates, query resolution, and strategic consultation to help you win more tenders.'
      },
      {
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        answer: 'Yes, we offer comprehensive training sessions on GeM portal navigation, bidding process, catalogue management, and best practices. Both one-on-one and group training options are available.'
      }
    ]
  }
];
            ...
            <div key={categoryIndex} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="divide-y divide-gray-200">
                {category.questions.map((item, questionIndex) => {
                  ...
                  const panelId = `faq-panel-${originalCategoryIndex}-${originalQuestionIndex}`;
                  const open = isOpen(originalCategoryIndex, originalQuestionIndex);

                  return (
                    <div key={questionIndex}>
                      <button
                        type="button"
                        id={`faq-btn-${originalCategoryIndex}-${originalQuestionIndex}`}
                        aria-expanded={open}
                      ...
                      >
                        ...
                        {open ? (
                          <ChevronUp size={24} className="text-[#0B3D91] flex-shrink-0" aria-hidden="true" />
                        ) : (
                          <ChevronDown size={24} className="text-gray-400 flex-shrink-0" aria-hidden="true" />
                        )}
                      </button>
                      {open && (
                        <div
                          id={panelId}
                          role="region"
                          aria-labelledby={`faq-btn-${originalCategoryIndex}-${originalQuestionIndex}`}
